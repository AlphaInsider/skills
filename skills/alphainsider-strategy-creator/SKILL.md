...
Guide one strategy from an idea to an automated AlphaInsider strategy.
...
- Check current applicable AlphaInsider limits and requirements before
  proposing or performing an AlphaInsider action. Recheck anything that can
  change immediately before the action.
...
- Make the generated plan and scheduled-run instructions sufficient for
  strategy runs and agreed self-healing without this installed skill. Use this
  skill for creation, updates, deletion, and automation reconfiguration.
- Treat performance as information. Poor performance never makes a
  run that follows `plan.md` unhealthy or authorizes a strategy change.
...
4. Route the requested work:
   ...
   - For any scheduled run, scheduler **Run now**, chat run, chat dry run,
     operational error, notification, or repair, read
...
A Draft plan permits interviewing, research into whether a reliable backtest
is possible, read-only discovery, and planning. Require the applicable plan to
be Agreed before a backtest or implementation build, change on AlphaInsider,
order-capable strategy run, or scheduler activation. Keep `plan.md` current
after each answer, material finding, completed action, failure, or next-step
change. Its status must let a new chat or scheduled agent continue without
this chat history.

If a backtest or implementation reveals that intended strategy behavior must
change, stop the affected work and return to the interview. Mechanical fixes
that do not change the agreed strategy, such as compatible endpoint wiring or
bounded rate-limit handling, do not need a new strategy decision.

An agreed automation plan authorizes later AlphaInsider paper orders that
follow the plan through its strategy run paths without per-order confirmation.
It does not authorize an unlisted change on AlphaInsider, broker action, or
strategy change.
