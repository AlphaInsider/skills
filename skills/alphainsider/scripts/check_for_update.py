#!/usr/bin/env python3
"""Silently check the canonical alphainsider wrapper version for an update."""
...
def fetch_remote_reference() -> str:
    request = Request(
        REMOTE_VERSION_URL,
        headers={"User-Agent": "AlphaInsider-Version-Check"},
    )
...
def update_notice(installed: str, available: str) -> str | None:
    if parse_semver(available) <= parse_semver(installed):
        return None
    return (
        f"alphainsider {available} is available (installed: {installed}).\n"
        "To update this skill, run:\n"
        f"{UPDATE_COMMAND}\n"
        "Then invoke alphainsider again."
    )
